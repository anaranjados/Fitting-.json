...
// window.onload=()=> {
	...
	let c = [
			{
				...
				"tel": "+7 8134 385123",
				"company": "Stolovaya #1",
				"addr": "fillingsoborsk, 10 part siezd, apt.94",
	...
	c.forEach(rec => {
		...
		function fill_row() {
			for (let i in arguments)
				filling.data[i].innerHTML = arguments[i]
		}
		
		for (let i=0; i<tab_row_n; i++)
			filling.data[i] = document.createElement('td')
		...
		for (let i=0; i<tab_row_n; i++) {
			...
			filling.row.appendChild(filling.data[i])
		}
		tab_body.appendChild( filling.row )
	})
